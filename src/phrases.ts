...
export default {
  ...
  roles: {
    ...
    membersFetchIncompleteReply:
      "Не удалось полностью загрузить участников сервера за отведённое время. Сообщение не создано — проверьте Server Members Intent у бота и попробуйте снова чуть позже.",
    refreshSkippedIncompleteMembersReply:
      "Сообщения не изменены: полный список участников не загрузился (частичный кэш исказил бы список). Повторите позже или проверьте нагрузку на шлюз Discord.",
    refreshNothingToDoReply:
      "Нечего обновлять: нет категорий ролей для этого сервера или не создано сообщение через команду сообщения ролей.",
  },
} as const;
